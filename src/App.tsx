...
const AwardCard: React.FC<{ ach: any }> = ({ ach }) => {
 ...
 >
 <div className={`w-full h-full transition-all duration-500 [transform-style:preserve-3d] ${flipped ? '[transform:rotateY(180deg)]' : ''} ${ach.acquired ? 'group-' : ''}`}>
 ...
 ach.acquired 
 ? 'bg-white border-slate-100 shadow-lg group-' 
...
const FlashcardLearning = ({ onFinish, onBack }: { onFinish: () => void, onBack: () => void }) => {
 ...
 <button onClick={onBack} className="w-[clamp(36px,10vw,48px)] h-[clamp(36px,10vw,48px)] bg-slate-100 rounded-[clamp(12px,3vw,16px)] flex items-center justify-center text-slate-600 transition-colors"><X size={28} /></button>
...
export default function App() {
 ...
 <div className="absolute inset-0 bg-yellow-400 rounded-[clamp(12px,3vw,16px)] rotate-3 group- transition-transform shadow-md"></div>
 <div className="w-[clamp(48px,14vw,64px)] h-[clamp(48px,14vw,64px)] bg-white rounded-[clamp(12px,3vw,16px)] p-1 shadow-lg border-2 border-white overflow-hidden relative z-10 group- transition-transform">
 ...
 <div onClick={() => setCurrentView('shop')} className="bg-white/90 backdrop-blur-sm px-[clamp(14px,3.5vw,20px)] py-[clamp(8px,2.5vw,10px)] rounded-full shadow-md flex items-center cursor-pointer transition-all border border-white/50">
 ...
 <button onClick={() => setCurrentView('subscriptions')} className="bg-amber-50 transition-colors rounded-[24px] p-[clamp(16px,4vw,24px)] flex items-center justify-between border-2 border-amber-100 group w-full">
 <div className="flex items-center space-x-[clamp(12px,3vw,16px)]">
 ...
 <ChevronRight className="text-amber-400 group- transition-colors" size={32} />
 </button>
 <button onClick={() => setCurrentView('reports')} className="bg-blue-50 transition-colors rounded-[24px] p-[clamp(16px,4vw,24px)] flex items-center justify-between border-2 border-blue-100 group w-full">
 <div className="flex items-center space-x-[clamp(12px,3vw,16px)]">
 <div className="w-[clamp(40px,12vw,56px)] h-[clamp(40px,12vw,56px)] bg-white rounded-full shadow-sm flex items-center justify-center group- transition-transform">
 ...
 <ChevronRight className="text-blue-400 group- transition-colors" size={32} />
 </button>
 </section>
 </div>
 ...
 <button className="px-[clamp(12px,3vw,16px)] py-1.5 rounded-[clamp(6px,1.5vw,8px)] font-bold text-[clamp(14px,3.5vw,16px)] text-slate-500 ">简单</button>
 ...
 <button className="px-[clamp(12px,3vw,16px)] py-1.5 rounded-[clamp(6px,1.5vw,8px)] font-bold text-[clamp(14px,3.5vw,16px)] text-slate-500 ">挑战</button>
 </div>
 </div>
 {/* 用户协议 */}
 ...
 <span className="font-black text-slate-700 text-[clamp(18px,4.5vw,20px)] group- transition-colors">用户协议</span>
 </div>
 <ChevronRight className="text-slate-400 group- transition-colors" size={24} />
 </button>
 {/* 联系客服 */}
 <button className="w-full flex items-center justify-between py-[clamp(6px,2vw,8px)] border-b border-slate-200/60 group">
 <div className="flex items-center space-x-[clamp(12px,3vw,16px)]">
 ...
 <span className="font-black text-slate-700 text-[clamp(18px,4.5vw,20px)] group- transition-colors">联系客服</span>
 </div>
 <ChevronRight className="text-slate-400 group- transition-colors" size={24} />
 </button>
 {/* 退出登录 */}
 ...
 <div key={item.id} className={`${item.light} rounded-[24px] p-[clamp(12px,3vw,16px)] flex flex-col relative overflow-hidden transition-transform cursor-pointer border-2 border-white shadow-sm`}>
 ...
 <button onClick={() => changeDay(-1)} className="p-[clamp(6px,2vw,8px)] rounded-full transition-colors"><ArrowLeft size={24}/></button>
 ...
 <button onClick={() => changeDay(1)} disabled={new Date(new Date(selectedDate).setDate(selectedDate.getDate()+1)) > new Date()} className="p-[clamp(6px,2vw,8px)] rounded-full transition-colors disabled:opacity-30"><ArrowRight size={24}/></button>
 </div>
